...
const NavbarDark = () => {
  ...
  const dropdownMenus = {
    ...
    individuals: [
      ...
      {
        title: "Credit Card",
        description: "Earn upto 4% bitcoin back",
        herf: "#",
        icon: BiCreditCard,
      },
    ],
  ...
  return (
    <>
      <nav className="fixed top-11 md:top-8 z-50 bg-[#0f0f0f] h-16 w-full border-b border-zinc-800 px-4 md:px-8">
        <div className="h-full w-full flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-8">
            {!openSubMenu ? (
              ...
              <div className="size-11 rounded-full bg-zinc-800 flex items-center justify-center">
                <button
                  onClick={() => {
                    setActiveMenu([]);
                    setOpenSubMenu(false);
                ...
                >
                  <BiChevronLeft size={40} className="text-gray-300" />
                </button>
              </div>
            )}
            ...
            <div className="hidden xl:flex items-center">
              {navLinks.map((link) => (
                ...
                >
                  <NavLink
                    to={link.link}
                    className="text-gray-100 px-3 py-3 rounded-full hover:bg-zinc-800 font-medium text-md"
                  >
                    {link.label}
                  </NavLink>

                  {link.withDropdown && activeDropdown === link.id && (
                    <div className="fixed h-dvh backdrop-blur-lg top-24 right-0 w-dvw z-30">
                      <div className="bg-[#0f0f0f] h-95 grid grid-cols-3 p-6 overflow-y-auto">
                        <div className="col-span-2 grid grid-cols-2 gap-3">
                          {dropdownMenus[link.id]?.map((item, idx) => (
                            <Link
                              key={idx}
                              to={item.herf}
                              className="flex items-start cursor-pointer gap-3 p-3 rounded-lg hover:bg-zinc-800 transition-colors"
                            >
          ...
          {/* Right Side Actions */}
          <div className="flex gap-2.5 text-gray-100">
            {!openSearch && (
              <button
                className="size-11 cursor-pointer flex items-center justify-center rounded-full bg-zinc-800 hover:bg-zinc-700"
            ...
            <button
              className="lg:hidden size-11 rounded-full flex items-center justify-center text-gray-100 bg-zinc-800 hover:bg-zinc-700"
              onClick={toggleMenu}
            >
        ...
        {openLang && (
          <>
            ...
            />
            <div className="fixed z-40 top-24 right-28 w-88 bg-[#0f0f0f] rounded-2xl shadow-2xl border border-zinc-800 flex flex-col overflow-hidden">
              <p className="text-sm font-semibold text-gray-400 px-5 pt-5 pb-3">
                Language and region
              </p>
              {/* Search */}
                  ...
                  return (
                    ...
                    >
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-400">{lang.region}</p>
                      </div>
                      {active && (
                        <svg
        ...
        {openSearch && (
          <>
            ...
            ></div>
            <div className="hidden md:flex fixed z-40 top-24 right-16 w-[min(480px,calc(100vw-2rem))] max-h-130 bg-[#0f0f0f] rounded-2xl shadow-2xl border border-zinc-800 flex-col overflow-hidden">
              {/* Filter tabs */}
              <div className="flex items-center gap-2 p-3 border-b border-zinc-800 overflow-x-auto scrollbar-hide shrink-0">
              ...
              <div className="overflow-y-auto flex-1">
                ...
                {getFilteredSearchData().crypto.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-500 tracking-widest px-4 pt-4 pb-2 uppercase">
                      Crypto
                    </p>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] font-semibold text-gray-400 bg-zinc-800 px-1.5 py-0.5 rounded-md">
                              #{coin.rank}
                            </span>
                          </div>
                        ...
                        <div className="text-right hidden sm:block">
                          <p className="text-xs text-gray-400">{coin.vol}</p>
                          <p className="text-xs text-gray-500">{coin.mcap}</p>
                        </div>
                  ...
                  <div>
                    ...
                    {getFilteredSearchData().stocks.map((stock) => (
                      ...
                      >
                        ...
                        {/* Vol + mcap */}
                        <div className="text-right hidden sm:block">
                          <p className="text-xs text-gray-400">{stock.vol}</p>
                          <p className="text-xs text-gray-500">{stock.mcap}</p>
                        </div>
            ...
            <div className="md:hidden fixed inset-0 z-50 bg-[#0f0f0f] flex flex-col">
              {/* Top bar: back button + search input */}
              <div className="flex items-center gap-3 px-3 py-3 border-b border-zinc-800 shrink-0">
                <button
                  onClick={() => setOpenSearch(false)}
                  className="size-9 flex items-center justify-center rounded-full bg-zinc-800 hover:bg-zinc-700 shrink-0"
                >
                  <BiChevronLeft size={24} className="text-gray-300" />
                </button>
                <Input
                  className="w-full"
                  size="sm"
              ...
              <div className="overflow-y-auto flex-1">
                ...
                {getFilteredSearchData().crypto.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-500 tracking-widest px-4 pt-4 pb-1 uppercase">
                      Crypto
                    </p>
                    {getFilteredSearchData().crypto.map((coin) => (
                      <Link
                        ...
                        to="/explore"
                        onClick={() => setOpenSearch(false)}
                        className="flex items-center gap-3 px-4 py-2.5 hover:bg-zinc-800 active:bg-zinc-700 transition-colors"
                      >
                        <img
                          src={coin.icon}
                          alt={coin.name}
      ...
      {openMenu && (
        <section className="h-dvh xl:hidden fixed top-0 z-40 w-full bg-[#0f0f0f]  pt-26 md:pt-22 flex flex-col">
          {/* ── Main nav list ── */}
          {!openSubMenu && !openMobileLang && (
            <>
                ...
                {navLinks.map((menu) =>
                  !menu.withDropdown ? (
                    <NavLink
                      ...
                      to={menu.link}
                      onClick={toggleMenu}
                      className="flex w-full items-center justify-between p-4 text-xl font-medium text-gray-100"
                    >
                      {menu.label}
                    </NavLink>
                  ) : (
              ...
              {/* Bottom bar: Globe + Sign in */}
              <div className="px-4 py-5 border-t border-zinc-800 flex items-center gap-3">
                <button
                  onClick={() => {
                    setLangSearch("");
                    setOpenMobileLang(true);
                  }}
                  className="size-10 rounded-full bg-zinc-800 hover:bg-zinc-700 flex items-center justify-center shrink-0"
                >
                  <BiWorld size={20} className="text-gray-300" />
                </button>
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              <div className="flex items-center gap-2 px-3 py-3 shrink-0">
                <button
                  ...
                  className="size-9 flex items-center justify-center rounded-full hover:bg-zinc-800 shrink-0"
                >
                  <BiChevronLeft size={24} className="text-gray-300" />
                </button>
              ...
              <div className="px-4 pb-3 shrink-0">
                <div className="flex items-center gap-2 bg-zinc-800 rounded-full px-3 py-2.5">
                  <BiSearch size={16} className="text-gray-400 shrink-0" />
                  <input
                    type="text"
                    value={langSearch}
                ...
                {filteredLangs.map((lang) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center justify-between px-5 py-3 hover:bg-zinc-800 active:bg-zinc-700 transition-colors"
                    >
                      <div className="text-left">
                        <p className="text-sm font-semibold text-white">
                          {lang.name}
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
                {activeMenu.map((list) => (
                  <NavLink
                    ...
                    key={list.title}
                    onClick={toggleMenu}
                    className="flex items-center gap-2 p-3 hover:bg-zinc-800 rounded-xl"
                  >
                    <div className="bg-zinc-800 size-8 text-gray-300 flex items-center justify-center rounded-md">
                      <list.icon size={23} />
                    </div>
